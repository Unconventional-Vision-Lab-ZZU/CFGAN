...
def gradient(input):
    ...
    x=input.shape[3]
    y=input.shape[0]
    input1 = input[:, 0, :, :] * 0.299 + input[:, 1, :, :] * 0.587 + input[:, 2, :, :] * 0.114
    input1 = input1.reshape(y, 1, x, x)
    conv_op = nn.Conv2d(1, 3, 3, bias=False, padding=0)
...
def hist_similar(x, y):
    ...
    return (torch.norm((torch.histc(x, 255, min=t_min, max=t_max) - torch.histc(y, 255, min=t_min, max=t_max)),
                       2)) / 255
