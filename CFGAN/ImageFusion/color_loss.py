...
class Blur(nn.Module):
    def __init__(self, nc):
        ...
        self.weight = nn.Parameter(data=kernel, requires_grad=False).to('cuda')

    def forward(self, x):
        if x.size(1) != self.nc:
            raise RuntimeError(
                "The channel of input [%d] does not match the preset channel [%d]" % (x.size(1), self.nc))
        x = F.conv2d(x, self.weight, stride=1, padding=10, groups=self.nc).to('cuda')
        return x
...
class ColorLoss(nn.Module):
    ...
    def forward(self, x1, x2):
        return torch.sum(torch.pow((x1 - x2), 2)).div(2 * x1.size()[0]).to("cuda")
